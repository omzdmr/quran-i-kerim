...
void main() {
  test('generated ids remain unique when two events share the same timestamp', () {
    ...
    final ledger = QadaFastingLedger().addDebt(days: 2, occurredOn: DateTime(2026, 3, 1), createdAt: timestamp).complete(occurredOn: DateTime(2026, 9, 21), createdAt: timestamp);
    final ids = ledger.entries.map((entry) => entry.id).toSet();
    expect(ids, hasLength(2));
    expect(ids.where((id) => id.endsWith('-2')), hasLength(1));
  ...
  test('oversized imported ids and notes are bounded', () {
    final oversizedId = List<String>.filled(129, 'x').join(); final oversizedNote = List<String>.filled(800, 'n').join();
    final invalidId = QadaFastingLedger.decode(jsonEncode(<String, Object?>{'formatVersion': 1, 'entries': <Object?>[<String, Object?>{'id': oversizedId, 'kind': 'debt', 'days': 1, 'occurredOn': '2026-03-01', 'createdAt': '2026-09-21T10:00:00Z'}]}));
    expect(invalidId.entries, isEmpty);
    final longNote = QadaFastingLedger.decode(jsonEncode(<String, Object?>{'formatVersion': 1, 'entries': <Object?>[<String, Object?>{'id': 'valid', 'kind': 'debt', 'days': 1, 'occurredOn': '2026-03-01', 'createdAt': '2026-09-21T10:00:00Z', 'note': oversizedNote}]}));
