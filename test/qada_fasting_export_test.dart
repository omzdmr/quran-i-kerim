...
void main() {
  ...
  QadaFastingLedger buildLedger() => QadaFastingLedger()
      .addDebt(days: 3, occurredOn: DateTime(2025, 3, 2), createdAt: created, sourceRamadanYear: 1446, estimatedSource: true, note: 'private, opening balance', id: 'debt')
      .complete(occurredOn: DateTime(2026, 9, 21), createdAt: created.add(const Duration(minutes: 1)), sourceRamadanYear: 1446, note: '=sensitive formula', id: 'done');

  test('default export preserves stable identity/provenance but omits notes', () {
    final csv = exporter.build(buildLedger());
    expect(csv, startsWith('record_id,created_at_utc,occurred_on,entry_type,days,balance_delta,source_ramadan_hijri,estimated_source'));
    ...
    expect(csv, contains('debt,2026-09-21T07:00:00.000Z,2025-03-02,debt,3,3,1446,true'));
    expect(csv, contains('done,2026-09-21T07:01:00.000Z,2026-09-21,completion,1,-1,1446,false'));
  });

  test('opt-in private notes are escaped and spreadsheet-safe', () {
    final csv = exporter.build(buildLedger(), includePrivateNotes: true);
  ...
  test('file service writes a dated UTF-8 file without private notes', () async {
    ...
    addTearDown(() async { await directory.delete(recursive: true); });
    final service = QadaFastingExportFileService(directoryProvider: () async => directory);
    final file = await service.createFile(buildLedger(), now: DateTime(2026, 9, 21));
    final content = await file.readAsString();
    expect(file.path, endsWith('qada-fasting-ledger-2026-09-21.csv'));
    expect(content, startsWith('\ufeffrecord_id,created_at_utc'));
