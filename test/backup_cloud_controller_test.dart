...
void main() {
  ...
  test('upload re-inspects local data immediately before writing', () async {
    ...
    final reading = (decoded['data'] as Map<String, dynamic>)['reading'] as Map<String, dynamic>;
  ...
  test('diverged remote is not overwritten without explicit permission', () async {
    final remoteJson = jsonEncode(<String, Object?>{
      ...
      'data': <String, Object?>{'reading': <String, Object?>{'last_surah': 18, 'last_ayah': 10}},
    });
    final store = _MemoryCloudStore(object: BackupCloudObject(content: remoteJson, revision: 'r1', updatedAt: DateTime.parse('2026-09-16T10:00:00Z')));
    final controller = controllerFor(store);
    addTearDown(controller.dispose);
    await controller.connect();
  ...
  test('restore re-reads reviewed remote, returns rollback receipt and preserves remote', () async {
    final store = _MemoryCloudStore(object: BackupCloudObject(content: _backupJson(surah: 18, ayah: 10), revision: 'r1', updatedAt: DateTime.parse('2026-09-16T10:00:00Z')));
    final controller = controllerFor(store);
    addTearDown(controller.dispose);
    await controller.connect();
    store.object = BackupCloudObject(content: _backupJson(surah: 36, ayah: 58), revision: 'r2', updatedAt: DateTime.parse('2026-09-16T11:00:00Z'));

    final preparation = await controller.prepareRemoteRestore();
    final receipt = await controller.restoreRemote(preparation: preparation!);
...
String _backupJson({required int surah, required int ayah}) => jsonEncode(<String, Object?>{
  'version': 3,
  'createdAt': '2026-09-16T10:00:00Z',
  'data': <String, Object?>{'reading': <String, Object?>{'last_surah': surah, 'last_ayah': ayah}},
});
...
class _FakeConnector implements BackupCloudConnector {
  ...
  int closeCalls = 0;
  @override
  Future<BackupCloudConnection> connect({bool interactive = true}) async => BackupCloudConnection(accountLabel: 'reader@example.com', store: store, close: () => closeCalls += 1);
  @override
...
class _MemoryCloudStore implements BackupCloudStore {
  ...
  @override
  Future<BackupCloudObject> write({required String content, required String? expectedRevision}) async {
    final current = object;
    if (current == null) {
      if (expectedRevision != null) throw const BackupCloudConflictException();
    } else if (current.revision != expectedRevision) {
      throw const BackupCloudConflictException();
    }
    writeCount += 1;
    final next = BackupCloudObject(content: content, revision: 'r${writeCount + 1}', updatedAt: DateTime.parse('2026-09-16T12:00:00Z'));
    object = next;
    return next;
  }
}
