...
void main() {
  ...
  test('portable backup round trip preserves qada event identity and private note', () async {
    ...
    final original = QadaFastingLedger()
        .addDebt(days: 3, occurredOn: DateTime(2026, 3, 1), createdAt: DateTime.utc(2026, 9, 21, 8), sourceRamadanYear: 1447, note: 'private provenance', id: 'debt-1447')
        .complete(occurredOn: DateTime(2026, 9, 21), createdAt: DateTime.utc(2026, 9, 21, 8, 1), id: 'done-1447');
  ...
  test('merge round trip does not duplicate qada events already on device', () async {
    SharedPreferences.setMockInitialValues(<String, Object>{});
    const store = QadaFastingStore();
    final original = QadaFastingLedger().addDebt(days: 2, occurredOn: DateTime(2026, 3, 1), createdAt: DateTime.utc(2026, 9, 21, 8), sourceRamadanYear: 1447, id: 'stable-event');
    await store.save(original);
    final encoded = await service.exportJson(now: DateTime.utc(2026, 9, 21, 9));
