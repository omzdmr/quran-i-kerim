...
void main() {
  test('successful cloud restore keeps its undo receipt when follow-up remote refresh fails', () async {
    ...
    addTearDown(() async { if (await tempRoot.exists()) await tempRoot.delete(recursive: true); });
    final store = _FailFourthReadStore(BackupCloudObject(
      content: jsonEncode(<String, Object?>{
        'version': 3,
        'createdAt': '2026-09-22T00:00:00Z',
        'data': <String, Object?>{'reading': <String, Object?>{'last_surah': 36, 'last_ayah': 58}},
      }),
      revision: 'remote-1',
    ...
    final controller = BackupCloudController(
      ...
      coordinatorFactory: (cloudStore) => BackupCloudCoordinator(store: cloudStore, restoreFileService: restoreService),
    );
    addTearDown(controller.dispose);

    await controller.connect(); // read #1
    final preparation = await controller.prepareRemoteRestore(); // read #2
    final receipt = await controller.restoreRemote(preparation: preparation!); // read #3, restore, read #4 fails
    ...
    expect(controller.lastError, isNull, reason: 'A post-commit status refresh is not a restore failure.');
...
class _FailFourthReadStore implements BackupCloudStore {
  _FailFourthReadStore(this.object);
  final BackupCloudObject object;
  int reads = 0;
  @override
  Future<BackupCloudObject?> read() async {
    reads += 1;
    if (reads == 4) throw StateError('simulated transient remote refresh failure');
    return object;
  }
  @override
