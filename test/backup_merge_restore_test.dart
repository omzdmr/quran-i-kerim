...
void main() {
  ...
  test('merge keeps local keys absent from import and applies imported keys', () async {
    ...
    await service.restoreDecoded(<String, Object?>{
      ...
      'data': <String, Object?>{'reading': <String, Object?>{'last_surah': 36}, 'preferences': <String, Object?>{}},
  ...
  test('qada merge keeps local history and adds remote-only events', () async {
    ...
    await service.restoreDecoded(<String, Object?>{
      'version': BackupManifest.schemaVersion,
      'createdAt': '2026-09-21T14:00:00Z',
      'data': <String, Object?>{'fasting': <String, Object?>{QadaFastingStore.preferenceKey: remote.encode()}},
    }, mode: BackupRestoreMode.merge);

    final restored = await const QadaFastingStore().load();
    expect(restored.entries.map((entry) => entry.id), <String>['local-debt', 'remote-debt']);
    expect(restored.remainingDays, 5);
    expect(restored.debtByRamadan[1446], 3);
    expect(restored.debtByRamadan[1447], 2);
  });
  ...
  test('qada merge keeps local event on immutable id collision', () async {
    ...
    expect(restored.entries.single.days, 3);
    expect(restored.entries.single.note, 'local private note');
  });

  test('qada merge refuses malformed remote ledger instead of erasing local history', () async {
    ...
    await service.restoreDecoded(<String, Object?>{
      'version': BackupManifest.schemaVersion,
      'createdAt': '2026-09-21T14:00:00Z',
      'data': <String, Object?>{'fasting': <String, Object?>{QadaFastingStore.preferenceKey: malformed}},
    }, mode: BackupRestoreMode.merge);

    final restored = await const QadaFastingStore().load();
    expect(restored.entries.single.id, 'safe-local');
    expect(restored.remainingDays, 4);
  });
}
