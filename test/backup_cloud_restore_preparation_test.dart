...
void main() {
  ...
  test('reviewed cloud revision cannot be silently replaced by another device', () async {
    ...
    await expectLater(
      controller.restoreRemote(preparation: preparation!, mode: BackupRestoreMode.replace),
      throwsA(isA<BackupCloudConflictException>()),
    );

    final prefs = await SharedPreferences.getInstance();
    expect(prefs.getInt('last_surah'), 2, reason: 'No data may change after the preview becomes stale.');
    expect(prefs.getInt('last_ayah'), 255);
    expect(prefs.getString('theme_mode'), 'dark');
    final backupDir = Directory('${tempRoot.path}${Platform.pathSeparator}quran_backups');
    expect(await backupDir.exists(), isFalse, reason: 'Conflict is rejected before restore/safety-snapshot side effects.');
  });
}

BackupCloudObject _object(String revision, {required int surah, required String locale}) => BackupCloudObject(
