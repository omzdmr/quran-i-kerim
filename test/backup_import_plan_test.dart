...
void main() {
  ...
  test('future fasting preferences remain visible beside expanded qada events', () {
    final ledger = QadaFastingLedger().addDebt(days: 1, occurredOn: DateTime(2026, 3, 1), createdAt: DateTime.utc(2026, 1, 1), id: 'debt');
    final plan = planner.build(
      currentSections: <String, Object?>{'fasting': <String, Object?>{QadaFastingStore.preferenceKey: ledger.encode(), 'future_fasting_pref': 'local'}},
      incomingSections: <String, Object?>{'fasting': <String, Object?>{QadaFastingStore.preferenceKey: ledger.encode(), 'future_fasting_pref': 'remote'}},
    );
    expect(plan.localRecords, 2); expect(plan.incomingRecords, 2); expect(plan.conflictingRecords, 1); expect(plan.hasConflicts, isTrue);
  });
}
