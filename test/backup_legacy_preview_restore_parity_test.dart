...
void main() {
  ...
  test('v3 replace preview and restore agree that v4 reading plans survive', () async {
    SharedPreferences.setMockInitialValues(<String, Object>{
      'last_surah': 9,
    ...
    expect(
      ...
      reason: 'The preview must never warn about data the declared schema cannot mutate.',
    );
  });

  test('current-schema replace preview and restore agree about omitted data removal', () async {
    SharedPreferences.setMockInitialValues(<String, Object>{
      'last_surah': 9,
      'reading_plan_state_v1': '{"active":{"preset":"quran30"}}',
    });
    ...
    final plan = await service.planImportJson(encoded);
    final readingPlans = plan.sectionImpacts.singleWhere(
      (impact) => impact.section == 'readingPlans',
    );
    expect(readingPlans.localOnlyRecords, 1);

    await service.restoreJson(encoded, mode: BackupRestoreMode.replace);
    final prefs = await SharedPreferences.getInstance();
    expect(prefs.containsKey('reading_plan_state_v1'), isFalse);
  });
}
