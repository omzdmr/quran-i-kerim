...
void main() {
  ...
  testWidgets('shows changed sections and destructive device-only count', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        locale: Locale('en'),
        home: Scaffold(body: BackupRestoreImpactList(plan: plan)),
      ),
    );
  ...
  testWidgets('exposes a single accessible summary for each impact row', (tester) async {
    ...
    expect(
      find.bySemanticsLabel('Yer imleri. Yalnız cihazda: 1'),
      findsOneWidget,
    );
    expect(
      find.bySemanticsLabel('Notlar. Çakışan: 1 · Yedekten gelecek: 1'),
      findsOneWidget,
    );
    handle.dispose();
  });
  ...
  testWidgets('announces when backup produces no record changes', (tester) async {
    ...
    await tester.pumpWidget(
      const MaterialApp(
        locale: Locale('en'),
        home: Scaffold(body: BackupRestoreImpactList(plan: unchanged)),
      ),
    );

    expect(
      find.bySemanticsLabel('This backup does not change records on this device.'),
      findsOneWidget,
    );
    handle.dispose();
  });
