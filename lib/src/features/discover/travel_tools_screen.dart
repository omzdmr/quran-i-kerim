...
class TravelToolsScreen extends StatelessWidget {
  ...
  Future<void> _pasteImport(BuildContext context, _TravelToolsCopy c) async {
    ...
    try {
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(c.importFailed)));
      return;
    }
    if (!context.mounted) return;
...
class _TravelToolsCopy {
  const _TravelToolsCopy(this.title, this.body, this.meetingPoint, this.meetingPointBody, this.packing, this.packingBody, this.privacy, this.export, this.exportCopied, this.import, this.importTitle, this.importInvalid, this.importDone, this.importFailed, this.cancel, this.restore, this.items, this.hasMeeting);
  final String title, body, meetingPoint, meetingPointBody, packing, packingBody, privacy, export, exportCopied, import, importTitle, importInvalid, importDone, importFailed, cancel, restore, items, hasMeeting;
  String importPreview(TravelToolsImportPreview preview) {
    final point = preview.meetingPoint;
    final pointLabel = point == null ? '—' : point.name.isNotEmpty ? point.name : point.address;
