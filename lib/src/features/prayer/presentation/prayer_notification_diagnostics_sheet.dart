...
Future<void> showPrayerNotificationDiagnosticsSheet({required BuildContext context, required bool notificationsEnabled}) => showModalBottomSheet<void>(context: context, showDragHandle: true, isScrollControlled: true, builder: (_) => PrayerNotificationDiagnosticsSheet(notificationsEnabled: notificationsEnabled));
...
class _PrayerNotificationDiagnosticsSheetState extends State<PrayerNotificationDiagnosticsSheet> {
  ...
  Future<void> _load() async {
    setState(() { _error = null; _diagnostics = null; });
    try {
      ...
      if (!mounted) return;
      setState(() { _diagnostics = diagnostics; _probeRecord = probe; });
    } catch (error) {
      if (!mounted) return;
...
class _DiagnosticRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(
    ...
    child: Padding(
      ...
      child: Row(children: [Icon(icon, size: 20), const SizedBox(width: 10), Expanded(child: Text(label)), const SizedBox(width: 12), Flexible(child: Text(value, textAlign: TextAlign.end, style: const TextStyle(fontWeight: FontWeight.w800)))]),
    ),
  );
}
