...
class _GoogleDriveBackupSectionState extends State<GoogleDriveBackupSection> {
  ...
  Future<void> _connect() async { HapticFeedback.selectionClick(); await _guard(() => _controller.connect()); }
  Future<void> _refresh() async { HapticFeedback.selectionClick(); await _guard(_controller.refresh); }

  Future<void> _upload({required bool overwrite}) async {
    HapticFeedback.selectionClick();
    if (overwrite && !await _confirm(titleKey: 'backupCloudUploadConfirmTitle', bodyKey: 'backupCloudUploadConfirmBody', actionKey: 'backupCloudUpload')) return;
    await _guard(() => _controller.uploadLocal(allowOverwrite: overwrite), refreshOnFailure: true);
  }

  Future<void> _replaceInvalid() async {
    HapticFeedback.selectionClick();
    if (!await _confirm(titleKey: 'backupCloudReplaceConfirmTitle', bodyKey: 'backupCloudReplaceConfirmBody', actionKey: 'backupCloudReplace')) return;
    await _guard(() => _controller.uploadLocal(allowOverwrite: true), refreshOnFailure: true);
  }

  Future<void> _restore() async {
    HapticFeedback.selectionClick();
    BackupCloudRestorePreparation? preparation;
    await _guard(() async { preparation = await _controller.prepareRemoteRestore(); });
    ...
    final mode = await showDialog<BackupRestoreMode>(
      ...
      builder: (_) => BackupRestoreDialog(preview: reviewed.preview, plan: reviewed.plan),
    );
    if (!mounted || mode == null) return;

    BackupRestoreReceipt? receipt;
    await _guard(() async {
      receipt = await _controller.restoreRemote(preparation: reviewed, mode: mode);
  ...
  Future<void> _signOut() async { HapticFeedback.selectionClick(); await _guard(_controller.signOut); }

  Future<void> _guard(Future<void> Function() action, {bool refreshOnFailure = false}) async {
    try {
      await action();
    } catch (_) {
      if (refreshOnFailure && _controller.connected) {
        try { await _controller.refresh(); } catch (_) {}
      }
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(context.l10n.text('backupCloudFailed'))));
    }
  }

  Future<bool> _confirm({required String titleKey, required String bodyKey, required String actionKey}) async {
    ...
    return await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.of(dialogContext).pop(true), child: Text(l10n.text(actionKey))),
        ],
      ),
    ) ?? false;
  }
  ...
  Widget build(BuildContext context) {
    if (!(widget.enabledOverride ?? BackupBuildConfig.googleDriveEnabled)) return const SizedBox.shrink();
    final l10n = context.l10n;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (_controller.accountLabel != null) Text(l10n.text('backupCloudConnectedAs').replaceAll('{account}', _controller.accountLabel!), style: TextStyle(color: scheme.onSurfaceVariant, fontSize: 12)),
          ])),
          if (_controller.connected) IconButton(tooltip: l10n.text('backupCloudSignOut'), onPressed: _controller.busy ? null : _signOut, icon: const Icon(Icons.logout_rounded)),
        ]),
...
class _CloudStatusLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final key = switch (state) { BackupCloudState.remoteEmpty => 'backupCloudRemoteEmpty', BackupCloudState.upToDate => 'backupCloudUpToDate', BackupCloudState.diverged => 'backupCloudDiverged', BackupCloudState.invalidRemote => 'backupCloudInvalid', null => 'backupCloudChecking' };
    final icon = switch (state) { BackupCloudState.remoteEmpty => Icons.cloud_queue_rounded, BackupCloudState.upToDate => Icons.cloud_done_outlined, BackupCloudState.diverged => Icons.sync_problem_rounded, BackupCloudState.invalidRemote => Icons.warning_amber_rounded, null => Icons.sync_rounded };
