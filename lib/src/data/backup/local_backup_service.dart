...
class LocalBackupService {
  ...
  final SharedPreferencesBackupAdapter adapter; final BackupPreviewParser previewParser; final BackupImportPlanner importPlanner; final BackupRestoreCoordinator restoreCoordinator;
  ...
  Map<String, Object?> _validatedSections(Object? decoded) {
    final preview = previewParser.parse(decoded); if (!preview.canRestore || decoded is! Map || preview.version == null) throw const FormatException('Backup is not restorable.');
    final rawData = decoded['data']; if (rawData is! Map) throw const FormatException('Backup data is invalid.'); final selected = <String, Object?>{};
    for (final entry in rawData.entries) { if (entry.key is! String) throw const FormatException('Backup section key is invalid.'); final section = entry.key as String; if (!BackupManifest.sectionsForVersion(preview.version!).contains(section)) continue; if (section == 'fasting') _validateFastingSection(entry.value); selected[section] = entry.value; }
    return Map<String, Object?>.unmodifiable(selected);
  }
  void _validateFastingSection(Object? rawSection) {
    if (rawSection is! Map) throw const FormatException('Fasting backup section is invalid.'); final encoded = rawSection['qada_fasting_ledger_v1']; if (encoded == null) return;
    final events = encoded is String ? _qadaEvents(_decodeMap(encoded)) : null;
    if (events == null || !_qadaEventSetCoherent(events)) throw const FormatException('Qada fasting ledger in backup is invalid.');
  }
  ...
  Map<String, Object?> _mergeSections(Map<String, Object?> current, Map<String, Object?> incoming) {
    ...
    for (final section in BackupManifest.includedSections) {
      ...
      if (currentSection is Map || incomingSection is Map) { final currentMap = _stringMap(currentSection); final incomingMap = _stringMap(incomingSection); final keys = <String>{...currentMap.keys, ...incomingMap.keys}; merged[section] = <String, Object?>{for (final key in keys) key: _mergePreferenceValue(section: section, key: key, current: currentMap[key], incoming: incomingMap[key], hasIncoming: incomingMap.containsKey(key))}; }
  ...
  Object? _mergeStringLists(Object? current, Object? incoming) { final local = current is List && current.every((value) => value is String) ? current.cast<String>() : null; final remote = incoming is List && incoming.every((value) => value is String) ? incoming.cast<String>() : null; if (local == null) return remote ?? incoming; if (remote == null) return local; final values = <String>{...local, ...remote}.toList()..sort((a, b) { final aInt = int.tryParse(a); final bInt = int.tryParse(b); if (aInt != null && bInt != null) return aInt.compareTo(bInt); return a.compareTo(b); }); return values; }
  ...
  Object? _mergePageProgress(Object? current, Object? incoming) { if (current is! String) return incoming; if (incoming is! String) return current; final currentDecoded = _decodeMap(current); final incomingDecoded = _decodeMap(incoming); if (currentDecoded == null) return incomingDecoded == null ? current : incoming; if (incomingDecoded == null) return current; final merged = <String, Object?>{}; final keys = <String>{...currentDecoded.keys.whereType<String>(), ...incomingDecoded.keys.whereType<String>()}; for (final key in keys) { final local = currentDecoded[key]; final remote = incomingDecoded[key]; if (local == null) merged[key] = remote; else if (remote == null) merged[key] = local; else merged[key] = _newerProgress(local, remote); } return jsonEncode(merged); }
  Object? _newerProgress(Object? local, Object? incoming) { if (local is! Map) return incoming; if (incoming is! Map) return local; DateTime? timestamp(Map value) => DateTime.tryParse(value['lastReviewedAt']?.toString() ?? '') ?? DateTime.tryParse(value['memorizedAt']?.toString() ?? ''); final localTime = timestamp(local); final incomingTime = timestamp(incoming); if (localTime == null) return incomingTime == null ? local : incoming; if (incomingTime == null) return local; return incomingTime.isAfter(localTime) ? incoming : local; }
  ...
  Object? _mergeJsonEventLists(Object? current, Object? incoming) { if (current is! String) return incoming; if (incoming is! String) return current; final currentDecoded = _decodeList(current); final incomingDecoded = _decodeList(incoming); if (currentDecoded == null) return incomingDecoded == null ? current : incoming; if (incomingDecoded == null) return current; final byId = <String, Map>{}; void addEvents(List<dynamic> events) { for (final event in events) { if (event is! Map || !_validPracticeEvent(event)) continue; byId[event['id'] as String] = event; } } addEvents(currentDecoded); addEvents(incomingDecoded); final merged = byId.values.toList(growable: false)..sort((a, b) => (b['occurredAt'] as String).compareTo(a['occurredAt'] as String)); return jsonEncode(merged.take(4000).toList(growable: false)); }
  ...
  List<Map>? _qadaEvents(Map? document) {
    if (document == null || document['formatVersion'] != 1 || document['entries'] is! List) return null; final rawEntries = document['entries'] as List; if (rawEntries.length > 20000) return null;
    final result = <Map>[]; final ids = <String>{};
    for (final raw in rawEntries) { if (raw is! Map) return null; final id = raw['id']; final kind = raw['kind']; final days = raw['days']; final occurredOn = raw['occurredOn']; final createdAt = raw['createdAt']; final sourceYear = raw['sourceRamadanYear']; final note = raw['note']; if (id is! String || id.trim().isEmpty || id.length > 128 || !ids.add(id) || kind is! String || !const {'debt', 'completion', 'correction'}.contains(kind) || days is! int || days == 0 || days.abs() > 3650 || occurredOn is! String || DateTime.tryParse(occurredOn) == null || createdAt is! String || DateTime.tryParse(createdAt) == null || (sourceYear != null && (sourceYear is! int || sourceYear < 1 || sourceYear > 9999)) || (note != null && note is! String)) return null; result.add(raw); }
    return result;
  }

  bool _qadaEventSetCoherent(List<Map> events) {
    var balance = 0; final debtByYear = <int, int>{}; final completionByYear = <int, int>{};
    for (final event in events) {
      final kind = event['kind'] as String; final days = event['days'] as int; final year = event['sourceRamadanYear'];
      balance += kind == 'completion' ? -days : days;
      if (year is int && kind == 'debt') debtByYear[year] = (debtByYear[year] ?? 0) + days;
      if (year is int && kind == 'completion') completionByYear[year] = (completionByYear[year] ?? 0) + days;
    }
    if (balance < 0 || balance > 3650) return false;
    for (final item in completionByYear.entries) { if (item.value > (debtByYear[item.key] ?? 0)) return false; }
    return true;
  }
}
