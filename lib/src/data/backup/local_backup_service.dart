...
class LocalBackupService {
  ...
  Future<String> exportJson({DateTime? now}) async =>
      (await createDocument(now: now)).encode();
  ...
  Future<BackupImportPlan> planImportJson(String encoded) async {
    ...
    if (version == null) {
      throw const FormatException('Backup schema version is invalid.');
    }
    final current = await adapter.captureSections();
    return importPlanner.build(
      currentSections: current,
      incomingSections: incoming,
      managedSections: BackupManifest.sectionsForVersion(version),
    );
  }
  ...
  void _validateFastingSection(Object? rawSection) {
    if (rawSection is! Map) {
      throw const FormatException('Fasting backup section is invalid.');
    }
    final encoded = rawSection['qada_fasting_ledger_v1'];
    if (encoded == null) return;
    final events = encoded is String ? _qadaEvents(_decodeMap(encoded)) : null;
    if (events == null || !_qadaEventSetCoherent(events)) {
      throw const FormatException('Qada fasting ledger in backup is invalid.');
    }
  }
  ...
  Object? _mergePreferenceValue({
    ...
    if (section == 'fasting' && key == 'qada_fasting_ledger_v1') {
      return _mergeQadaLedger(current, incoming);
    }
    return incoming;
  }

  Object? _mergeStringLists(Object? current, Object? incoming) {
    final local = current is List && current.every((value) => value is String)
        ? current.cast<String>()
        : null;
    final remote = incoming is List && incoming.every((value) => value is String)
        ? incoming.cast<String>()
        : null;
  ...
  Object? _mergePageProgress(Object? current, Object? incoming) {
    ...
    if (currentDecoded == null) {
      return incomingDecoded == null ? current : incoming;
    }
  ...
  bool _validPracticeEvent(Map event) {
    ...
    return id is String &&
        ...
        page > 0 &&
        context is String &&
        const {'soloReview', 'prayer', 'recitedToSomeone'}.contains(context) &&
  ...
  Object? _mergeQadaLedger(Object? current, Object? incoming) {
    if (current is! String) return incoming;
    if (incoming is! String) return current;
    final localEvents = _qadaEvents(_decodeMap(current));
    final remoteEvents = _qadaEvents(_decodeMap(incoming));
    ...
    for (final event in remoteEvents) {
      ...
      if (!_sameQadaEventIdentity(existing, event)) {
        throw const FormatException(
          'Merged qada fasting history contains conflicting event ids.',
        );
      }
      if (existing['note'] == null && event['note'] is String) {
        byId[id] = event;
      }
    }
    ...
    if (!_qadaEventSetCoherent(events)) {
      throw const FormatException(
        'Merged qada fasting history would be inconsistent.',
      );
    }
  ...
  bool _sameQadaEventIdentity(Map a, Map b) {
    bool identityKey(Object? key) =>
        key is String && key != 'note' && key != 'createdAt';
    final left = <String, Object?>{
      for (final entry in a.entries)
        if (identityKey(entry.key)) entry.key as String: entry.value,
    };
  ...
  List<Map>? _qadaEvents(Map? document) {
    ...
    final rawEntries = document['entries'] as List;
    if (rawEntries.length > 20000) return null;
    final result = <Map>[];
    final ids = <String>{};
    for (final raw in rawEntries) {
      ...
      final kind = raw['kind'];
      final days = raw['days'];
      final occurredOn = raw['occurredOn'];
      final createdAt = raw['createdAt'];
      final sourceYear = raw['sourceRamadanYear'];
      final note = raw['note'];
      if (id is! String ||
          id.trim().isEmpty ||
          id.length > 128 ||
          !ids.add(id) ||
          kind is! String ||
          !const {'debt', 'completion', 'correction'}.contains(kind) ||
          days is! int ||
          days == 0 ||
          days.abs() > 3650 ||
          occurredOn is! String ||
          DateTime.tryParse(occurredOn) == null ||
          createdAt is! String ||
          DateTime.tryParse(createdAt) == null ||
          (sourceYear != null &&
              (sourceYear is! int || sourceYear < 1 || sourceYear > 9999)) ||
  ...
  bool _qadaEventSetCoherent(List<Map> events) {
    ...
    for (final event in events) {
      ...
      final year = event['sourceRamadanYear'];
      balance += kind == 'completion' ? -days : days;
      if (year is int && kind == 'debt') {
        debtByYear[year] = (debtByYear[year] ?? 0) + days;
      }
      if (year is int && kind == 'completion') {
        completionByYear[year] = (completionByYear[year] ?? 0) + days;
      }
    }
    if (balance < 0 || balance > 3650) return false;
    for (final item in completionByYear.entries) {
      if (item.value > (debtByYear[item.key] ?? 0)) return false;
    }
    return true;
  }
}
