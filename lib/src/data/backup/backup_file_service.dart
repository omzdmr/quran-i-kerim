...
class BackupFileService {
  ...
  Future<File> exportToFile({DateTime? now}) async {
    ...
    final stamp = createdAt.toIso8601String().replaceAll('-', '').replaceAll(':', '').replaceAll('.', '');
    final target = File('${directory.path}${Platform.pathSeparator}quran-backup-$stamp.json');
  ...
  Future<List<File>> listBackupFiles() async {
    ...
    final files = await directory.list(followLinks: false).where((entry) => entry is File && _fileName(entry.path).startsWith('quran-backup-') && entry.path.endsWith('.json')).cast<File>().toList();
  ...
  Future<void> restoreFile(File file, {BackupRestoreMode mode = BackupRestoreMode.replace}) async {
    final encoded = await _readImport(file);
  ...
  Future<String> _readImport(File file) async {
    if (maxImportBytes <= 0) throw ArgumentError.value(maxImportBytes, 'maxImportBytes', 'must be > 0');
    if (!await file.exists()) throw FileSystemException('Backup file does not exist.', file.path);
    final length = await file.length();
    if (length > maxImportBytes) throw FormatException('Backup file exceeds the $maxImportBytes byte import limit.');
