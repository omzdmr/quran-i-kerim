...
Native heading now tracks physical iPhone/iPad orientation and updates `CLLocationManager.headingOrientation` when the device rotates. This prevents a landscape iPad or rotated iPhone from feeding a portrait-relative compass heading to the shared Qibla UI. `getCapabilityStatus` exposes `orientationAwareHeading: true`; Qibla bearing calculation itself remains shared-layer owned.
