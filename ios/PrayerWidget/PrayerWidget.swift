...
private enum SnapshotReader {
  ...
  static func read(now: Date = Date(), timeZone: TimeZone = .autoupdatingCurrent, locale: Locale = .autoupdatingCurrent) -> Result {
    guard let defaults = UserDefaults(suiteName: suiteName), let data = defaults.data(forKey: payloadKey) else { return Result(snapshot: nil, freshness: nil) }
    guard let snapshot = try? JSONDecoder().decode(PrayerSnapshot.self, from: data) else { return Result(snapshot: nil, freshness: .malformed) }
    let freshness = snapshot.freshness(now: now, currentTimeZone: timeZone, currentLocale: locale)
...
private struct PrayerProvider: TimelineProvider {
  ...
  func getTimeline(in context: Context, completion: @escaping (Timeline<PrayerEntry>) -> Void) {
    ...
    if let snapshot = result.snapshot {
      ...
      let ageLimit = Date(timeIntervalSince1970: (snapshot.generatedAt + PrayerSnapshot.maximumAgeMilliseconds) / 1000)
      if expiry > now { refresh = min(refresh, expiry) }
      if ageLimit > now { refresh = min(refresh, ageLimit) }
    ...
    } else if result.freshness != nil { refresh = now.addingTimeInterval(5 * 60) }
...
private struct PrayerWidgetView: View {
  ...
  @ViewBuilder private var privacyRedacted: some View {
    if #available(iOSApplicationExtension 16.0, *), family == .accessoryInline { Text(String(localized: "Prayer times hidden", table: "Localizable")).accessibilityLabel(String(localized: "Prayer times hidden for privacy", table: "Localizable")) }
    else if #available(iOSApplicationExtension 16.0, *), family == .accessoryCircular { Image(systemName: "lock.fill").accessibilityLabel(String(localized: "Prayer times hidden for privacy", table: "Localizable")) }
    else if #available(iOSApplicationExtension 16.0, *), family == .accessoryRectangular { Text(String(localized: "Prayer times hidden", table: "Localizable")).font(.caption).accessibilityLabel(String(localized: "Prayer times hidden for privacy", table: "Localizable")) }
    else { VStack(alignment: .leading, spacing: 4) { Text(String(localized: "Prayer times hidden", table: "Localizable")).font(.headline); Text(String(localized: "Prayer times hidden for privacy", table: "Localizable")).font(.caption).foregroundStyle(.secondary) }.accessibilityElement(children: .combine) }
  }

  @ViewBuilder private var unavailable: some View {
    if #available(iOSApplicationExtension 16.0, *), family == .accessoryInline { Text(unavailableMessage).lineLimit(1) }
    else if #available(iOSApplicationExtension 16.0, *), family == .accessoryCircular { Image(systemName: "arrow.clockwise").accessibilityLabel(unavailableMessage) }
    else if #available(iOSApplicationExtension 16.0, *), family == .accessoryRectangular { Text(unavailableMessage).font(.caption).lineLimit(2) }
    else { VStack(alignment: .leading, spacing: 4) { Text(String(localized: "Prayer Times", table: "Localizable")).font(.headline); Text(unavailableMessage).font(.caption).foregroundStyle(.secondary) }.accessibilityElement(children: .combine) }
  }

  private var unavailableMessage: String {
    switch entry.freshness {
    case .timeZoneChanged: return String(localized: "Prayer times need refresh after a time zone change", table: "Localizable")
    case .localeChanged: return String(localized: "Open app to refresh", table: "Localizable")
    case .generatedTooOld, .expired, .prayerBoundaryPassed: return String(localized: "Prayer information is out of date. Open the app to refresh", table: "Localizable")
    case .malformed, .unsupportedSchema, .generatedInFuture: return String(localized: "Prayer information cannot be shown safely. Open the app to refresh", table: "Localizable")
    case .fresh, .none: return String(localized: "Open app to refresh", table: "Localizable")
    }
  }
  ...
  @ViewBuilder private func widgetBackground<Content: View>(@ViewBuilder content: () -> Content) { if isAccessoryFamily { content() } else if #available(iOS 17.0, *) { content().containerBackground(for: .widget) { Color(uiColor: .secondarySystemBackground) }.padding() } else { content().padding().background(Color(uiColor: .secondarySystemBackground)) } }
}
