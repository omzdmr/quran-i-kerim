...
private struct PrayerEntry: TimelineEntry {
  ...
  var canShowDetails: Bool {
    guard let snapshot, case .fresh = snapshot.freshness(at: date) else { return false }
...
private struct PrayerProvider: TimelineProvider {
  ...
  func getTimeline(in context: Context, completion: @escaping (Timeline<PrayerEntry>) -> Void) {
    ...
    if let boundary = nextBoundary(snapshot: snapshot, after: now) { entries.append(PrayerEntry(date: boundary, snapshot: snapshot)) }
  ...
  private func nextRefresh(snapshot: PrayerSnapshot?, now: Date) -> Date {
    guard let boundary = nextBoundary(snapshot: snapshot, after: now) else { return now.addingTimeInterval(snapshot == nil ? 300 : 60) }
    return max(now.addingTimeInterval(60), min(boundary, now.addingTimeInterval(1800)))
  }
  private func load() -> PrayerSnapshot? {
    guard let data = UserDefaults(suiteName: appGroup)?.data(forKey: snapshotKey), let snapshot = try? decoder.decode(PrayerSnapshot.self, from: data), snapshot.version == 1 else { return nil }
    return snapshot
  }
}
...
private struct PrayerWidgetView: View {
  ...
  private func prayerContent(_ snapshot: PrayerSnapshot, _ nextAt: Date) -> some View {
    ...
    case .accessoryInline:
      Label { Text("\(name) \(nextAt, style: .timer)").lineLimit(1) } icon: { Image(systemName: "clock") }
    ...
    case .accessoryCircular:
      VStack(spacing: 1) { Image(systemName: "clock").font(.caption2).accessibilityHidden(true); Text(nextAt, style: .timer).font(.caption2.monospacedDigit()).minimumScaleFactor(0.65) }
        .privacySensitive().accessibilityElement(children: .ignore)
        .accessibilityLabel(Text("\(name), \(nextAt.formatted(date: .omitted, time: .shortened))"))
    ...
    case .systemMedium:
      HStack(spacing: 14) {
        VStack(alignment: .leading, spacing: 4) {
          Label(name, systemImage: "moon.stars.fill").font(.headline).lineLimit(1)
          Text(nextAt, style: .time).font(.title3.weight(.semibold)).monospacedDigit()
        }
        Spacer(minLength: 8)
        VStack(alignment: .trailing, spacing: 2) {
          Text("Time remaining").font(.caption).foregroundStyle(.secondary)
          Text(nextAt, style: .timer).font(.title2.monospacedDigit()).minimumScaleFactor(0.7).lineLimit(1)
        }
      }
      .privacySensitive().accessibilityElement(children: .ignore)
      .accessibilityLabel(Text("\(name), \(nextAt.formatted(date: .omitted, time: .shortened))"))
  ...
  private var staleContent: some View {
    if family == .accessoryCircular || family == .accessoryInline {
      Label(entry.snapshot?.privacyMode == "redacted" ? "Prayer times hidden" : "Open app to refresh", systemImage: entry.snapshot?.privacyMode == "redacted" ? "lock.fill" : "arrow.clockwise")
  ...
  private func stateContent(icon: String, title: LocalizedStringKey, accessibility: LocalizedStringKey) -> some View {
    VStack(alignment: .leading, spacing: 5) { Image(systemName: icon).accessibilityHidden(true); Text(title).font(.headline) }
