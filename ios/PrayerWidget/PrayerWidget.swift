...
private struct PrayerWidgetView: View {
  ...
  @ViewBuilder private func prayerContent(_ snapshot: PrayerSnapshot, _ nextAt: Date) -> some View {
    ...
    case .systemExtraLarge:
      HStack(spacing: 32) {
        VStack(alignment: .leading, spacing: 14) { Label(name, systemImage: "moon.stars.fill").font(.title.weight(.semibold)).lineLimit(2); Spacer(minLength: 0); Text(nextAt, style: .time).font(.system(size: 52, weight: .semibold, design: .rounded)).monospacedDigit().minimumScaleFactor(0.6).lineLimit(1); Spacer(minLength: 0) }
        Divider().accessibilityHidden(true)
        VStack(alignment: .leading, spacing: 10) { Image(systemName: "clock").font(.title).accessibilityHidden(true); Text("Time remaining").font(.title3).foregroundStyle(.secondary); Text(nextAt, style: .timer).font(.system(size: 44, weight: .medium, design: .rounded)).monospacedDigit().minimumScaleFactor(0.55).lineLimit(1); Spacer(minLength: 0) }.frame(maxWidth: .infinity, alignment: .leading)
  ...
  @ViewBuilder private var unavailableContent: some View {
    if entry.snapshot?.privacyMode == "redacted" {
      if family == .accessoryCircular || family == .accessoryInline { Label { Text("Prayer times hidden") } icon: { Image(systemName: "lock.fill") }.accessibilityLabel(Text("Prayer times hidden for privacy")) } else { stateContent(icon: "lock.fill", title: "Prayer times hidden", accessibility: "Prayer times hidden for privacy") }
