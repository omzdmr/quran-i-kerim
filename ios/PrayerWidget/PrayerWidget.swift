...
private struct PrayerProvider: TimelineProvider {
  ...
  func getTimeline(in context: Context, completion: @escaping (Timeline<PrayerEntry>) -> Void) {
    let now = Date(), result = SnapshotReader.read(now: now); var refresh = now.addingTimeInterval(15 * 60)
    if let snapshot = result.snapshot { let expiry = Date(timeIntervalSince1970: snapshot.validUntil / 1000); if expiry > now { refresh = min(refresh, expiry) }; if let next = snapshot.nextPrayerAt { let boundary = Date(timeIntervalSince1970: next / 1000); if boundary > now { refresh = min(refresh, boundary) } } }
...
private struct PrayerWidgetView: View {
  ...
  @available(iOSApplicationExtension 16.0, *) private func inline(id: String, at: Double) -> some View {
    HStack(spacing: 3) { Text(localizedPrayerName(id)); Text("·").accessibilityHidden(true); Text(time(at)).monospacedDigit() }
      .lineLimit(1).accessibilityElement(children: .ignore).accessibilityLabel(accessibilityLabel(id: id, at: at))
  }
  ...
  @available(iOSApplicationExtension 16.0, *) private func rectangular(id: String, at: Double) -> some View {
    VStack(alignment: .leading, spacing: 2) { Text(String(localized: "Prayer Times", table: "Localizable")).font(.caption2).foregroundStyle(.secondary).lineLimit(1); HStack(spacing: 5) { Text(localizedPrayerName(id)); Text(time(at)).monospacedDigit() }.font(.headline).minimumScaleFactor(0.75) }
  ...
  @ViewBuilder private var unavailable: some View {
    if #available(iOSApplicationExtension 16.0, *), family == .accessoryInline { Text(String(localized: "Open app to refresh", table: "Localizable")) }
    else if #available(iOSApplicationExtension 16.0, *), family == .accessoryCircular { Image(systemName: "arrow.clockwise").accessibilityLabel(String(localized: "Open app to refresh", table: "Localizable")) }
    else if #available(iOSApplicationExtension 16.0, *), family == .accessoryRectangular { Text(String(localized: "Open app to refresh", table: "Localizable")).font(.caption) }
    else { VStack(alignment: .leading, spacing: 4) { Text(String(localized: "Prayer Times", table: "Localizable")).font(.headline); Text(String(localized: "Open app to refresh", table: "Localizable")).font(.caption).foregroundStyle(.secondary) }.accessibilityElement(children: .combine) }
  }
