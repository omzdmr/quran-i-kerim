...
final class MicrophoneRecordingChannel: NSObject, AVAudioRecorderDelegate {
  ...
  func detach() {
    ...
    recorder = nil; activeURL = nil; startedAt = nil
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    switch call.method {
    case "capabilities": result(["format": "m4a/aac", "metering": true, "persistentUserData": true, "listAndDelete": true])
  ...
  private func startRecording(_ arguments: Any?, result: @escaping FlutterResult) {
    guard session.recordPermission == .granted else {
      result(FlutterError(code: "microphone_permission_required", message: "Microphone permission is required before recording.", details: nil)); return
    }
    guard recorder?.isRecording != true else {
      result(FlutterError(code: "recording_already_active", message: "A recitation recording is already active.", details: nil)); return
    }
    do {
      ...
      let settings: [String: Any] = [
        AVFormatIDKey: Int(kAudioFormatMPEG4AAC), AVSampleRateKey: 44_100.0, AVNumberOfChannelsKey: 1,
        AVEncoderAudioQualityKey: AVAudioQuality.high.rawValue, AVEncoderBitRateKey: 96_000
      ]
      ...
      self.recorder = recorder; activeURL = url; startedAt = Date()
  ...
  private func stopRecording(discard: Bool, result: @escaping FlutterResult) {
    guard let recorder, let url = activeURL else {
      result(FlutterError(code: "no_active_recording", message: "There is no active recording.", details: nil)); return
    }
    recorder.stop()
    let duration = recorder.currentTime
    self.recorder = nil; activeURL = nil; startedAt = nil
    AudioSessionCoordinator.shared.configurePolicy()
    if discard {
      try? FileManager.default.removeItem(at: url); result(["discarded": true])
    } else {
      result(["recording": false, "recordingId": url.deletingPathExtension().lastPathComponent, "path": url.path, "durationSeconds": max(0, duration), "sizeBytes": fileSize(url)])
    }
  }
  ...
  private func statusPayload() -> [String: Any] {
    ...
    var payload: [String: Any] = [
      "recording": true, "path": url.path, "durationSeconds": max(0, recorder.currentTime),
      "averagePowerDb": recorder.averagePower(forChannel: 0), "peakPowerDb": recorder.peakPower(forChannel: 0)
    ]
    if let startedAt { payload["startedAtMs"] = Int64(startedAt.timeIntervalSince1970 * 1000) }
    return payload
  }
  ...
  private func listRecordings(result: @escaping FlutterResult) {
    do {
      let directory = try recordingsDirectory()
      let keys: Set<URLResourceKey> = [.fileSizeKey, .creationDateKey, .contentModificationDateKey, .isRegularFileKey]
      ...
      let items: [[String: Any]] = urls.compactMap { url in
        ...
        if let created = values.creationDate { item["createdAtMs"] = Int64(created.timeIntervalSince1970 * 1000) }
        if let modified = values.contentModificationDate { item["modifiedAtMs"] = Int64(modified.timeIntervalSince1970 * 1000) }
        return item
  ...
  private func deleteRecording(_ arguments: Any?, result: @escaping FlutterResult) {
    guard let args = arguments as? [String: Any], let rawID = args["recordingId"] as? String, let id = sanitizedIdentifier(rawID), id == rawID else {
      result(FlutterError(code: "invalid_recording_id", message: "A valid recordingId is required.", details: nil)); return
    }
    do {
      let directory = try recordingsDirectory()
      let url = directory.appendingPathComponent("\(id).m4a").standardizedFileURL
      guard url.deletingLastPathComponent() == directory.standardizedFileURL else { throw RecordingError.invalidPath }
      guard FileManager.default.fileExists(atPath: url.path) else {
        result(FlutterError(code: "recording_not_found", message: "The recording does not exist.", details: nil)); return
      }
      if activeURL?.standardizedFileURL == url {
        result(FlutterError(code: "recording_is_active", message: "Stop or cancel the active recording before deleting it.", details: nil)); return
      }
      try FileManager.default.removeItem(at: url)
      result(["deleted": true, "recordingId": id])
  ...
  private func uniqueURL(in directory: URL, base: String) -> URL {
    var candidate = directory.appendingPathComponent("\(base).m4a"); var index = 2
    while FileManager.default.fileExists(atPath: candidate.path) { candidate = directory.appendingPathComponent("\(base)-\(index).m4a"); index += 1 }
    return candidate
  }
  ...
  func audioRecorderEncodeErrorDidOccur(_ recorder: AVAudioRecorder, error: Error?) {
    guard let error else { return }; channel.invokeMethod("recordingError", arguments: ["message": error.localizedDescription])
  }

  func audioRecorderDidFinishRecording(_ recorder: AVAudioRecorder, successfully flag: Bool) {
    guard !flag else { return }; channel.invokeMethod("recordingError", arguments: ["message": "Recording stopped before the file was finalized."])
  }
  ...
  private enum RecordingError: LocalizedError {
    case couldNotStart, invalidPath
    var errorDescription: String? {
      switch self { case .couldNotStart: return "The audio recorder could not start."; case .invalidPath: return "The recording path is invalid." }
    }
  }
}
