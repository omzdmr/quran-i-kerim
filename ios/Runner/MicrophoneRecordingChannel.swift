...
final class MicrophoneRecordingChannel: NSObject, AVAudioRecorderDelegate {
  ...
  private let channel: FlutterMethodChannel, session: AVAudioSession, center: NotificationCenter
  private var observers: [NSObjectProtocol] = [], recorder: AVAudioRecorder?, activeURL: URL?, startedAt: Date?, interruptedAt: Date?, activeInputUID: String?
  ...
  init(binaryMessenger: FlutterBinaryMessenger, session: AVAudioSession = .sharedInstance(), notificationCenter: NotificationCenter = .default) {
    channel = FlutterMethodChannel(name: Self.channelName, binaryMessenger: binaryMessenger); self.session = session; center = notificationCenter; super.init(); channel.setMethodCallHandler { [weak self] call, result in self?.handle(call, result: result) }; observeAudioSession()
  }
  func detach() { observers.forEach(center.removeObserver); observers.removeAll(); if recorder?.isRecording == true { recorder?.stop() }; recorder = nil; activeURL = nil; startedAt = nil; interruptedAt = nil; activeInputUID = nil; channel.setMethodCallHandler(nil); AudioSessionCoordinator.shared.configurePolicy() }
  private func observeAudioSession() { observers = [center.addObserver(forName: AVAudioSession.interruptionNotification, object: session, queue: .main) { [weak self] note in self?.handleInterruption(note) }, center.addObserver(forName: AVAudioSession.routeChangeNotification, object: session, queue: .main) { [weak self] note in self?.handleRouteChange(note) }] }
  ...
  private func startRecording(_ arguments: Any?, result: @escaping FlutterResult) {
    ...
    do {
      ...
      self.recorder = recorder; activeURL = url; startedAt = Date(); interruptedAt = nil; activeInputUID = session.currentRoute.inputs.first?.uid
  ...
  private func stopRecording(discard: Bool, result: @escaping FlutterResult) {
    ...
    else { result(completedPayload(url: url, duration: duration, recording: false)) }
  }

  private func statusPayload() -> [String: Any] {
    guard let recorder, let url = activeURL else { return ["recording": false] }; recorder.updateMeters()
    ...
    if let startedAt { payload["startedAtMs"] = Int64(startedAt.timeIntervalSince1970 * 1000) }; if let interruptedAt { payload["interruptedAtMs"] = Int64(interruptedAt.timeIntervalSince1970 * 1000) }; if let activeInputUID { payload["inputUID"] = activeInputUID }
  ...
  private func handleInterruption(_ notification: Notification) {
    ...
    case .ended:
      guard let recorder, let url = activeURL, interruptedAt != nil else { return }; let optionsRaw = (notification.userInfo?[AVAudioSessionInterruptionOptionKey] as? UInt) ?? 0, options = AVAudioSession.InterruptionOptions(rawValue: optionsRaw); recorder.stop(); let duration = max(0, recorder.currentTime); resetActiveRecording(); var payload = completedPayload(url: url, duration: duration, recording: false); payload["canRestart"] = options.contains(.shouldResume); channel.invokeMethod("recordingInterrupted", arguments: payload)
