...
final class MicrophoneRecordingChannel: NSObject, AVAudioRecorderDelegate {
  ...
  private func observeAudioSession() {
    observers = [
      center.addObserver(forName: AVAudioSession.interruptionNotification, object: session, queue: .main) { [weak self] note in self?.handleInterruption(note) },
      center.addObserver(forName: AVAudioSession.routeChangeNotification, object: session, queue: .main) { [weak self] note in self?.handleRouteChange(note) }
    ]
  }

  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    switch call.method {
    case "capabilities": result(["format": "m4a/aac", "metering": true, "persistentUserData": true, "listAndDelete": true, "interruptionRecovery": true, "routeChangeEvents": true])
  ...
  private func statusPayload() -> [String: Any] {
    ...
    if let interruptedAt { payload["interruptedAtMs"] = Int64(interruptedAt.timeIntervalSince1970 * 1000) }
    return payload
  }
  ...
  private func handleInterruption(_ notification: Notification) {
    ...
    case .began:
      ...
      interruptedAt = Date()
      channel.invokeMethod("recordingInterruptionChanged", arguments: ["interrupted": true, "atMs": Int64(Date().timeIntervalSince1970 * 1000)])
    case .ended:
      guard let recorder, let url = activeURL, interruptedAt != nil else { return }
      let optionsRaw = (notification.userInfo?[AVAudioSessionInterruptionOptionKey] as? UInt) ?? 0
      let options = AVAudioSession.InterruptionOptions(rawValue: optionsRaw)
      // Never silently resume a recitation capture. A resumed microphone could record speech
      // the user did not intend to capture. Finalize the partial file and let UI explicitly restart.
      recorder.stop()
      let duration = max(0, recorder.currentTime)
      resetActiveRecording()
      channel.invokeMethod("recordingInterrupted", arguments: [
        "recordingId": url.deletingPathExtension().lastPathComponent,
        "path": url.path,
        "durationSeconds": duration,
        "sizeBytes": fileSize(url),
        "canRestart": options.contains(.shouldResume)
      ])
    @unknown default:
      break
    }
  ...
  private func handleRouteChange(_ notification: Notification) {
    ...
    channel.invokeMethod("recordingRouteChanged", arguments: [
      "reason": Self.routeChangeName(reason),
  ...
  private func listRecordings(result: @escaping FlutterResult) {
    do {
      ...
      let items: [[String: Any]] = urls.compactMap { url in
        ...
        var item: [String: Any] = ["recordingId": url.deletingPathExtension().lastPathComponent, "path": url.path, "sizeBytes": values.fileSize ?? 0]
  ...
  func audioRecorderDidFinishRecording(_ finishedRecorder: AVAudioRecorder, successfully flag: Bool) {
    ...
    let url = activeURL
    let duration = max(0, finishedRecorder.currentTime)
    resetActiveRecording()
  ...
  private enum RecordingError: LocalizedError {
    ...
    var errorDescription: String? { switch self { case .couldNotStart: return "The audio recorder could not start."; case .invalidPath: return "The recording path is invalid." } }
  }
}
