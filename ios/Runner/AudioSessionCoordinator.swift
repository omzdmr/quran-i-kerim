...
final class NowPlayingCoordinator {
  ...
  func start() {
    ...
    register(commandCenter.playCommand, command: .play); register(commandCenter.pauseCommand, command: .pause); register(commandCenter.nextTrackCommand, command: .next); register(commandCenter.previousTrackCommand, command: .previous)
    let seekTarget = commandCenter.changePlaybackPositionCommand.addTarget { [weak self] event in guard let self, self.commandCenter.changePlaybackPositionCommand.isEnabled, let seekEvent = event as? MPChangePlaybackPositionCommandEvent, self.onRemoteCommand != nil else { return .commandFailed }; self.onRemoteCommand?(.seek, seekEvent.positionTime); return .success }
  ...
  func update(_ metadata: Metadata) {
    let duration = metadata.duration.flatMap { $0.isFinite && $0 > 0 ? $0 : nil }
    let elapsed = duration.map { min(max(0, metadata.elapsed), $0) } ?? max(0, metadata.elapsed)
    var info: [String: Any] = [MPMediaItemPropertyTitle: metadata.title, MPMediaItemPropertyMediaType: MPMediaType.audio.rawValue, MPNowPlayingInfoPropertyElapsedPlaybackTime: elapsed, MPNowPlayingInfoPropertyPlaybackRate: metadata.playbackRate, MPNowPlayingInfoPropertyDefaultPlaybackRate: 1.0]
    ...
    commandCenter.changePlaybackPositionCommand.isEnabled = duration != nil
    commandCenter.nextTrackCommand.isEnabled = metadata.canGoNext
    commandCenter.previousTrackCommand.isEnabled = metadata.canGoPrevious
