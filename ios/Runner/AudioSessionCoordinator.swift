...
final class NowPlayingCoordinator {
  ...
  func update(_ metadata: Metadata) {
    ...
    commandCenter.playCommand.isEnabled = true; commandCenter.pauseCommand.isEnabled = true; commandCenter.changePlaybackPositionCommand.isEnabled = duration != nil; commandCenter.nextTrackCommand.isEnabled = metadata.canGoNext; commandCenter.previousTrackCommand.isEnabled = metadata.canGoPrevious; infoCenter.nowPlayingInfo = info
    if #available(iOS 13.0, *) { infoCenter.playbackState = metadata.playbackRate > 0 ? .playing : .paused }
  }
  func clear() { infoCenter.nowPlayingInfo = nil; disablePlaybackCommands(); if #available(iOS 13.0, *) { infoCenter.playbackState = .stopped } }
  ...
  private func register(_ remote: MPRemoteCommand, command: RemoteCommand) { let target = remote.addTarget { [weak self, weak remote] _ in guard let self, remote?.isEnabled == true, self.onRemoteCommand != nil else { return .commandFailed }; self.onRemoteCommand?(command, nil); return .success }; commandTargets.append((remote, target)) }
}
