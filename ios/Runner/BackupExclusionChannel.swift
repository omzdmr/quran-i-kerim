...
final class BackupExclusionChannel {
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    guard let arguments = call.arguments as? [String: Any], let storageArea = arguments["storageArea"] as? String, let relativePath = arguments["relativePath"] as? String else {
      result(FlutterError(code: "invalid_backup_exclusion_arguments", message: "storageArea and relativePath are required.", details: nil)); return
    }
    do {
      let status: BackupExclusionCoordinator.Status
      if call.method == Self.excludeMethod { status = try coordinator.exclude(storageArea: storageArea, relativePath: relativePath) }
      else { status = try coordinator.status(storageArea: storageArea, relativePath: relativePath) }
...
final class NowPlayingChannel {
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "updateNowPlaying":
      guard let args = call.arguments as? [String: Any], let title = args["title"] as? String, !title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
        result(FlutterError(code: "invalid_now_playing_metadata", message: "A non-empty title is required.", details: nil)); return
      }
      ...
      guard elapsed.isFinite, elapsed >= 0, rate.isFinite, rate >= 0, duration == nil || (duration!.isFinite && duration! > 0) else {
        result(FlutterError(code: "invalid_now_playing_timing", message: "Playback timing values are invalid.", details: nil)); return
      }
      coordinator.update(.init(title: title, subtitle: args["subtitle"] as? String, albumTitle: args["albumTitle"] as? String, duration: duration, elapsed: elapsed, playbackRate: rate))
      result(nil)
...
/// Sends native AVAudioSession lifecycle events to the shared player without
/// making native code decide whether playback should resume. This preserves the
/// user's last explicit play/pause intent across calls, alarms and route loss.
final class AudioLifecycleChannel {
  ...
  init(binaryMessenger: FlutterBinaryMessenger, notificationCenter: NotificationCenter = .default) {
    ...
    channel = FlutterMethodChannel(name: Self.name, binaryMessenger: binaryMessenger)
    observers = [
      notificationCenter.addObserver(forName: AudioSessionCoordinator.interruptionBeganNotification, object: nil, queue: .main) { [weak channel] _ in
        channel?.invokeMethod("interruptionBegan", arguments: nil)
      },
      notificationCenter.addObserver(forName: AudioSessionCoordinator.interruptionEndedNotification, object: nil, queue: .main) { [weak channel] note in
        let shouldResume = note.userInfo?["shouldResume"] as? Bool ?? false
        channel?.invokeMethod("interruptionEnded", arguments: ["shouldResume": shouldResume])
      },
      notificationCenter.addObserver(forName: AudioSessionCoordinator.routeChangedNotification, object: nil, queue: .main) { [weak channel] note in
        let reason = note.userInfo?["reason"] as? UInt ?? 0
        channel?.invokeMethod("routeChanged", arguments: ["reason": reason])
      },
    ]
  }
