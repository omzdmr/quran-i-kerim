...
struct WidgetPrayerSnapshot: Codable, Equatable {
  ...
  private var structurallyValid: Bool {
    ...
    return generatedAt < validUntil && TimeZone(identifier: timeZoneIdentifier) != nil && !calculationFingerprint.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty && hasPrayerID == hasPrayerTime && (privacyMode == .redacted || hasPrayerID) && prayerTimeIsPlausible
  }
...
final class WidgetSnapshotStore {
  ...
  func save(_ snapshot: WidgetPrayerSnapshot, now: Date = Date(), currentTimeZone: TimeZone = .autoupdatingCurrent) throws {
    guard snapshot.version == WidgetPrayerSnapshot.schemaVersion, snapshot.freshness(at: now, currentTimeZone: currentTimeZone) == .fresh else { throw StoreError.invalidSnapshot }
  ...
  @discardableResult func purgeIfStale(now: Date = Date(), timeZone: TimeZone = .autoupdatingCurrent) -> Bool {
    ...
    let freshness = snapshot.freshness(at: now, currentTimeZone: timeZone), reason: InvalidationReason
    switch freshness { case .fresh: return false; case .generatedInFuture: reason = .generatedInFuture; case .expired: reason = .expired; case .timeZoneChanged: reason = .timeZoneChanged; case .prayerBoundaryPassed: reason = .prayerBoundaryPassed; case .malformed, .unsupportedSchema: reason = .corruptPayload }
