...
/// Versioned, privacy-aware snapshot shared with WidgetKit/Lock Screen extensions.
/// The app owns snapshot production; extensions only consume this compact projection.
...
final class WidgetSnapshotStore {
  ...
  func save(_ snapshot: WidgetPrayerSnapshot, now: Date = Date()) throws {
    guard snapshot.validUntil > now,
          ...
          !snapshot.timeZoneIdentifier.isEmpty,
          !snapshot.calculationFingerprint.isEmpty else {
