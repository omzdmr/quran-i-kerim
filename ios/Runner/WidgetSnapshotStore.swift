...
final class WidgetSnapshotStore {
  ...
  enum StoreError: LocalizedError { case appGroupUnavailable, invalidSnapshot; var errorDescription: String? { switch self { case .appGroupUnavailable: return "Shared widget storage is unavailable."; case .invalidSnapshot: return "The widget snapshot is invalid or already expired." } } }
  ...
  func save(_ snapshot: WidgetPrayerSnapshot, now: Date = Date()) throws { guard let snapshotTimeZone = TimeZone(identifier: snapshot.timeZoneIdentifier), snapshot.version == WidgetPrayerSnapshot.schemaVersion, snapshot.validUntil > now, snapshot.generatedAt <= now.addingTimeInterval(60), snapshot.freshness(at: max(now, snapshot.generatedAt), currentTimeZone: snapshotTimeZone) != .malformed else { throw StoreError.invalidSnapshot }; defaults.set(try encoder.encode(snapshot), forKey: Self.snapshotKey); defaults.removeObject(forKey: Self.lastInvalidationKey) }
  ...
  @discardableResult func purgeIfStale(now: Date = Date(), timeZone: TimeZone = .autoupdatingCurrent) -> Bool {
    let hadStoredPayload = defaults.object(forKey: Self.snapshotKey) != nil
    guard let snapshot = validatedLoad(now: now, timeZone: timeZone) else { return hadStoredPayload && defaults.object(forKey: Self.snapshotKey) == nil }
    let freshness = snapshot.freshness(at: now, currentTimeZone: timeZone)
    let reason: InvalidationReason
    switch freshness {
    case .fresh: return false
    case .generatedInFuture: reason = .generatedInFuture
    case .expired: reason = .expired
    case .timeZoneChanged: reason = .timeZoneChanged
    case .prayerBoundaryPassed: reason = .prayerBoundaryPassed
    case .malformed, .unsupportedSchema: reason = .corruptPayload
    }
    invalidate(reason); return true
  }
