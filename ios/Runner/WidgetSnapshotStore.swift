...
final class WidgetSnapshotStore {
  ...
  func save(_ snapshot: WidgetPrayerSnapshot, now: Date = Date()) throws {
    guard let snapshotTimeZone = TimeZone(identifier: snapshot.timeZoneIdentifier), snapshot.version == WidgetPrayerSnapshot.schemaVersion, snapshot.validUntil > now, snapshot.generatedAt <= now.addingTimeInterval(60), snapshot.freshness(at: max(now, snapshot.generatedAt), currentTimeZone: snapshotTimeZone) != .malformed else { throw StoreError.invalidSnapshot }
