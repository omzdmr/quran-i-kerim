...
struct WidgetPrayerSnapshot: Codable, Equatable {
  ...
  enum Freshness: String, Equatable {
    case fresh, unsupportedSchema, malformed, generatedInFuture, expired, timeZoneChanged, prayerBoundaryPassed
  }
  ...
  private var structurallyValid: Bool {
    ...
    return generatedAt < validUntil &&
      ...
      hasPrayerID == hasPrayerTime && prayerTimeIsPlausible
  }
...
final class WidgetSnapshotStore {
  ...
  init?(suiteName: String = appGroupIdentifier) {
    ...
    self.defaults = defaults; encoder = JSONEncoder(); decoder = JSONDecoder(); configureCoders()
  }
  ...
  func save(_ snapshot: WidgetPrayerSnapshot, now: Date = Date()) throws {
    guard snapshot.version == WidgetPrayerSnapshot.schemaVersion,
          ...
          snapshot.freshness(at: max(now, snapshot.generatedAt), currentTimeZone: TimeZone(identifier: snapshot.timeZoneIdentifier)!) != .malformed else { throw StoreError.invalidSnapshot }
  ...
  func load() -> WidgetPrayerSnapshot? {
    ...
    guard let snapshot = try? decoder.decode(WidgetPrayerSnapshot.self, from: data) else { return nil }
    return snapshot
  }

  /// Reads and validates persisted state. Corrupt or structurally impossible payloads are quarantined
  /// immediately so extensions never keep retrying data that cannot become valid with time.
  ...
  @discardableResult func purgeIfStale(now: Date = Date(), timeZone: TimeZone = .autoupdatingCurrent) -> Bool {
    guard let snapshot = validatedLoad(now: now, timeZone: timeZone) else { return false }
    guard !snapshot.isFresh(at: now, currentTimeZone: timeZone) else { return false }
    invalidate(.stale); return true
  }

  @discardableResult func purgeIfPolicyChanged(currentFingerprint: String) -> Bool {
    let normalized = currentFingerprint.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !normalized.isEmpty, let snapshot = validatedLoad(), snapshot.calculationFingerprint != normalized else { return false }
    invalidate(.policyChanged); return true
  }
