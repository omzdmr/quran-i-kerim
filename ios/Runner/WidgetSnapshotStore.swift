...
struct WidgetPrayerSnapshot: Codable, Equatable {
  ...
  private var structurallyValid: Bool {
    ...
    let prayerTimeIsPlausible = nextPrayerAt.map {
      $0 > generatedAt && $0 <= validUntil
    } ?? true
  ...
  func freshness(
    ...
    guard timeZoneIdentifier == currentTimeZone.identifier else {
      return .timeZoneChanged
    }
    if let nextPrayerAt, nextPrayerAt <= date {
      return .prayerBoundaryPassed
    }
    return .fresh
  }
  ...
  func presentation(
    ...
    guard freshness(at: date, currentTimeZone: currentTimeZone) == .fresh else {
      return .stale
    }
...
final class WidgetSnapshotStore {
  ...
  enum StoreError: LocalizedError {
    ...
    var errorDescription: String? {
      ...
      case .invalidSnapshot:
        return "The widget snapshot is invalid, stale or already expired."
      }
    }
  }
  ...
  func save(_ snapshot: WidgetPrayerSnapshot, now: Date = Date()) throws {
    guard
      let snapshotTimeZone = TimeZone(identifier: snapshot.timeZoneIdentifier),
      snapshot.version == WidgetPrayerSnapshot.schemaVersion,
      snapshot.freshness(at: now, currentTimeZone: snapshotTimeZone) == .fresh
    else {
  ...
  func validatedLoad(
    ...
    guard let snapshot = load() else {
      invalidate(.corruptPayload)
      return nil
    }

    let freshness = snapshot.freshness(at: now, currentTimeZone: timeZone)
    if freshness == .malformed || freshness == .unsupportedSchema {
      invalidate(.corruptPayload)
      return nil
    }
    return snapshot
  ...
  func freshness(
    now: Date = Date(),
    timeZone: TimeZone = .autoupdatingCurrent
  ) -> WidgetPrayerSnapshot.Freshness? {
    validatedLoad(now: now, timeZone: timeZone)?
      .freshness(at: now, currentTimeZone: timeZone)
  }

  func presentation(
    now: Date = Date(),
    timeZone: TimeZone = .autoupdatingCurrent
  ) -> WidgetPrayerSnapshot.Presentation {
    validatedLoad(now: now, timeZone: timeZone)?
      .presentation(at: now, currentTimeZone: timeZone) ?? .stale
  }
  ...
  func purgeIfStale(
    ...
    guard let snapshot = validatedLoad(now: now, timeZone: timeZone) else {
      return hadStoredPayload && defaults.object(forKey: Self.snapshotKey) == nil
    }
    ...
    case .fresh:
      return false
    case .generatedInFuture:
      reason = .generatedInFuture
    case .expired:
      reason = .expired
    case .timeZoneChanged:
      reason = .timeZoneChanged
    case .prayerBoundaryPassed:
      reason = .prayerBoundaryPassed
  ...
  func purgeIfPolicyChanged(currentFingerprint: String) -> Bool {
    ...
    guard
      !normalized.isEmpty,
      let snapshot = validatedLoad(),
      snapshot.calculationFingerprint != normalized
    else {
      return false
    }
    invalidate(.policyChanged)
    return true
  }

  func clear() {
    invalidate(.explicitClear)
  }

  func lastInvalidationReason() -> String? {
    defaults.string(forKey: Self.lastInvalidationKey)
  }

  private func invalidate(_ reason: InvalidationReason) {
    defaults.removeObject(forKey: Self.snapshotKey)
    defaults.set(reason.rawValue, forKey: Self.lastInvalidationKey)
  }
}
