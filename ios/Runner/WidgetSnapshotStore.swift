...
final class WidgetSnapshotStore {
  ...
  func save(_ snapshot: WidgetPrayerSnapshot, now: Date = Date()) throws { guard let snapshotTimeZone = TimeZone(identifier: snapshot.timeZoneIdentifier), snapshot.version == WidgetPrayerSnapshot.schemaVersion, snapshot.freshness(at: now, currentTimeZone: snapshotTimeZone) == .fresh else { throw StoreError.invalidSnapshot }; defaults.set(try encoder.encode(snapshot), forKey: Self.snapshotKey); defaults.removeObject(forKey: Self.lastInvalidationKey) }
