...
final class WidgetSnapshotChannel {
  ...
  init(binaryMessenger: FlutterBinaryMessenger, store: WidgetSnapshotStore? = WidgetSnapshotStore(), notificationCenter: NotificationCenter = .default) {
    ...
    self.store = store; self.notificationCenter = notificationCenter
  ...
  private func invalidatePolicy(_ arguments: Any?, result: @escaping FlutterResult) {
    ...
    result(invalidationPayload(purged: purged))
  }
  ...
  private func statusPayload() -> [String: Any] {
    ...
    guard let snapshot = store.validatedLoad() else {
      var payload: [String: Any] = ["available": true, "hasSnapshot": false, "fresh": false, "freshness": "missing"]
      if let reason = store.lastInvalidationReason() {
        payload["lastInvalidationReason"] = reason
        if reason == WidgetSnapshotStore.InvalidationReason.corruptPayload.rawValue { reloadWidgets() }
      }
      return payload
    }
