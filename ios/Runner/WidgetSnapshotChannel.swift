...
final class WidgetSnapshotChannel {
  ...
  private func installFreshnessObservers() { let names: [Notification.Name] = [UIApplication.significantTimeChangeNotification, NSNotification.Name.NSSystemTimeZoneDidChange, UIApplication.didBecomeActiveNotification]; observers = names.map { name in notificationCenter.addObserver(forName: name, object: nil, queue: .main) { [weak self] notification in self?.handleFreshnessEvent(notification) } } }
  private func removeFreshnessObservers() { observers.forEach(notificationCenter.removeObserver); observers.removeAll() }
  private func handleFreshnessEvent(_ notification: Notification) { guard let store else { return }; let purged = store.purgeIfStale(now: Date(), timeZone: .autoupdatingCurrent); if purged || notification.name != UIApplication.didBecomeActiveNotification { reloadWidgets() } }
  ...
  private func invalidatePolicy(_ arguments: Any?, result: @escaping FlutterResult) {
    ...
    let purged = store?.purgeIfPolicyChanged(currentFingerprint: fingerprint) ?? false; if purged { reloadWidgets() }; result(invalidationPayload(purged: purged))
  }

  private func invalidationPayload(purged: Bool) -> [String: Any] {
    var payload: [String: Any] = ["purged": purged]
    if let reason = store?.lastInvalidationReason() { payload["reason"] = reason }
    return payload
  }
  ...
  private func publish(_ arguments: Any?, result: @escaping FlutterResult) {
    ...
    do { try store.save(snapshot); reloadWidgets(); result(["stored": true, "validUntilMs": validUntilMs, "freshness": WidgetPrayerSnapshot.Freshness.fresh.rawValue]) } catch { result(FlutterError(code: "snapshot_rejected", message: error.localizedDescription, details: nil)) }
  }

  private func statusPayload() -> [String: Any] {
    guard let store else { return ["available": false, "hasSnapshot": false, "fresh": false, "freshness": "missing"] }
    guard let snapshot = store.validatedLoad() else { var payload: [String: Any] = ["available": true, "hasSnapshot": false, "fresh": false, "freshness": "missing"]; if let reason = store.lastInvalidationReason() { payload["lastInvalidationReason"] = reason }; return payload }
