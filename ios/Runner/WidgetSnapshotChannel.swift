  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "capabilities":
      result([
        ...
        "stalePurge": true,
        "corruptPayloadQuarantine": true,
        "invalidationDiagnostics": true,
  ...
  private func invalidatePolicy(_ arguments: Any?, result: @escaping FlutterResult) {
    guard
      ...
      result(FlutterError(
        ...
        message: "invalidatePolicy requires calculationFingerprint.",
        details: nil
      ))
      return
    }
  ...
  private func publish(_ arguments: Any?, result: @escaping FlutterResult) {
    guard let store else {
      result(FlutterError(
        ...
        message: "Shared widget storage is unavailable.",
        details: nil
      ))
      return
    }
    guard
      ...
      result(FlutterError(
        code: "invalid_arguments",
        message: "publish requires generatedAtMs, validUntilMs, timeZone and calculationFingerprint.",
        details: nil
      ))
      return
    }
    ...
    guard let privacy = WidgetPrayerSnapshot.PrivacyMode(rawValue: privacyRaw) else {
      result(FlutterError(
        ...
        message: "privacyMode must be standard or redacted.",
        details: nil
      ))
      return
    }
  ...
  private func statusPayload() -> [String: Any] {
    ...
    guard let snapshot = store.validatedLoad() else {
      ...
      if let reason = store.lastInvalidationReason() {
        ...
        if reason == WidgetSnapshotStore.InvalidationReason.corruptPayload.rawValue {
          reloadWidgets()
        }
      }
      return payload
