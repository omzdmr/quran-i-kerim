...
final class WidgetSnapshotChannel {
  ...
  private func installFreshnessObservers() {
    let names: [Notification.Name] = [UIApplication.significantTimeChangeNotification, NSNotification.Name.NSSystemTimeZoneDidChange, NSLocale.currentLocaleDidChangeNotification, UIApplication.didBecomeActiveNotification]
  ...
  private func handleFreshnessEvent(_ notification: Notification) { guard let store else { return }; let purged = store.purgeIfStale(now: Date(), timeZone: .autoupdatingCurrent, locale: .autoupdatingCurrent); if purged || notification.name != UIApplication.didBecomeActiveNotification { reloadWidgets() } }
  ...
  private func invalidatePolicy(_ arguments: Any?, result: @escaping FlutterResult) { guard let args = arguments as? [String: Any], let fingerprint = args["calculationFingerprint"] as? String, !fingerprint.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { result(FlutterError(code: "invalid_arguments", message: "invalidatePolicy requires calculationFingerprint.", details: nil)); return }; let purged = store?.purgeIfPolicyChanged(currentFingerprint: fingerprint) ?? false; if purged { reloadWidgets() }; result(invalidationPayload(purged: purged)) }
  private func invalidationPayload(purged: Bool) -> [String: Any] { var payload: [String: Any] = ["purged": purged]; if let reason = store?.lastInvalidationReason() { payload["reason"] = reason }; return payload }
  private func publish(_ arguments: Any?, result: @escaping FlutterResult) {
    ...
    let privacyRaw = (args["privacyMode"] as? String) ?? "standard"; guard let privacy = WidgetPrayerSnapshot.PrivacyMode(rawValue: privacyRaw) else { result(FlutterError(code: "invalid_privacy_mode", message: "privacyMode must be standard or redacted.", details: nil)); return }
    let nextPrayerMs = number(args["nextPrayerAtMs"])
    let snapshot = WidgetPrayerSnapshot(generatedAt: Date(timeIntervalSince1970: generatedMs / 1000), validUntil: Date(timeIntervalSince1970: validUntilMs / 1000), timeZoneIdentifier: timeZone, localeIdentifier: Locale.autoupdatingCurrent.identifier, calculationFingerprint: fingerprint, nextPrayerID: args["nextPrayerId"] as? String, nextPrayerAt: nextPrayerMs.map { Date(timeIntervalSince1970: $0 / 1000) }, displayName: args["displayName"] as? String, privacyMode: privacy)
    do { try store.save(snapshot); reloadWidgets(); result(["stored": true, "validUntilMs": validUntilMs, "freshness": WidgetPrayerSnapshot.Freshness.fresh.rawValue, "locale": snapshot.localeIdentifier]) } catch { result(FlutterError(code: "snapshot_rejected", message: error.localizedDescription, details: nil)) }
  }
...
#if canImport(WidgetKit)
    if #available(iOS 14.0, *) { WidgetCenter.shared.reloadTimelines(ofKind: Self.widgetKind) }
#endif
  }
}
