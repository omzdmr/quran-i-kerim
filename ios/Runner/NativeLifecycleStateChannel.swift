...
final class NativeLifecycleStateChannel {
  ...
  private func observeLifecycle() {
    observers = [
      ...
      center.addObserver(forName: UIApplication.willEnterForegroundNotification, object: nil, queue: .main) { [weak self] _ in self?.record(Key.lastForegroundAt); self?.emit("foreground") },
      center.addObserver(forName: UIApplication.didBecomeActiveNotification, object: nil, queue: .main) { [weak self] _ in self?.emit("active") },
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "capabilities": result(["coldLaunchDetection": true, "firstLaunchDistinction": true, "backgroundGap": true, "lifecycleEvents": true, "persistent": true])
  ...
  private func emit(_ state: String) { channel.invokeMethod("lifecycleChanged", arguments: ["state": state, "atMs": milliseconds(Date())]) }
