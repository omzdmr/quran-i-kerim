...
/// termination recovery and protected-data availability without requiring a backend.
final class NativeLifecycleStateChannel {
  ...
  private let channel: FlutterMethodChannel, defaults: UserDefaults, center: NotificationCenter
  private var observers: [NSObjectProtocol] = []
  private let hadPreviousSession: Bool, previousCleanTermination: Bool, launchedAt = Date()

  init(binaryMessenger: FlutterBinaryMessenger, defaults: UserDefaults = .standard, notificationCenter: NotificationCenter = .default) {
    channel = FlutterMethodChannel(name: Self.channelName, binaryMessenger: binaryMessenger); self.defaults = defaults; center = notificationCenter
    let previousLaunchCount = defaults.integer(forKey: Key.launchCount)
    hadPreviousSession = previousLaunchCount > 0; previousCleanTermination = hadPreviousSession && defaults.bool(forKey: Key.cleanTermination)
    defaults.set(false, forKey: Key.cleanTermination); defaults.set(previousLaunchCount + 1, forKey: Key.launchCount); defaults.set(TimeZone.current.identifier, forKey: Key.lastKnownTimeZone)
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "capabilities": result(["coldLaunchDetection": true, "firstLaunchDistinction": true, "backgroundGap": true, "lifecycleEvents": true, "persistent": true, "timeZoneEvents": true, "significantTimeChange": true, "widgetStaleReconciliation": true, "protectedDataAvailability": true])
  ...
  private func handleTimeZoneChange() { let previous = defaults.string(forKey: Key.lastKnownTimeZone), current = TimeZone.current.identifier; defaults.set(current, forKey: Key.lastKnownTimeZone); let purged = purgeStalePrayerProjection(); emit("timeZoneChanged", extras: ["previousTimeZone": previous ?? "", "timeZone": current, "widgetSnapshotPurged": purged]) }
