...
private func expectInvalid(
  _ snapshot: WidgetPrayerSnapshot,
...
struct WidgetSnapshotValidation {
  static func main() throws {
    ...
    expectInvalid(
      ...
      store: store,
      now: now,
      "future-generated publish rejects"
    )
    expectInvalid(
      WidgetPrayerSnapshot(
        ...
        calculationFingerprint: "policy",
        nextPrayerID: "fajr",
        nextPrayerAt: now.addingTimeInterval(-1),
        displayName: "Fajr",
        privacyMode: .standard
      ),
      store: store,
      now: now,
      "already-passed prayer publish rejects"
    )
